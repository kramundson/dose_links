...
```{r}
head(dosage_genos_trifilt)
```

```{r}
# can probably do split-apply-combine more elegantly
...
derpy <- dosage_genos %>% 
  ...
  # filter(n > 3) %>%  # removes outliers, may need to change this threshold
  # filter(n < 160) # removes monomorphic, may need to change this threshold
# table(derpy$n)
  # group_by(bin, dosage.gt) %>% 
  # summarize(dosage.gt.count = )
View(derpy)
```

```{r}
# generate all nonredundant comparisons from derpy
derpy_crossed <- derpy %>% 
  setNames(paste0("target.", names(.))) %>% 
  crossing(derpy) %>% 
  filter(target.bin != bin & target.dosage.gt != dosage.gt) %>% # remove nonself, know that redundant comparisons will still be there
  filter(target.n > 3) %>% 
  filter(n > 3) %>%
  mutate(comparison = ifelse(target.bin > bin, paste(target.bin, bin, sep = "_"), paste(bin, target.bin, sep = "_")))

# not the tidyverse way, but won't crash like my attempt above did.
# problem is I don't know how to sort and paste on the fly like this
# maybe an ifelse?
# for (i in 1:nrow(derpy_crossed)) {
#   derpy_crossed$comparison[i] <- paste(sort(c(derpy_crossed$target.bin[i], derpy_crossed$bin[i])), collapse = "_")
# }

# then, filter by comparison
```

```{r}
# derpy_crossed$comparison <- rep(NA, nrow(derpy_crossed))

# for (i in 1:nrow(derpy_crossed$))
# 
# paste(sort(c(derpy_crossed$target.bin[1], derpy_crossed$bin[1])), collapse = "_")
# 
# for (i in 1:nrow(derpy_crossed)) {
#   derpy_crossed$comparison[i] <- paste(sort(c(derpy_crossed$target.bin[i], derpy_crossed$bin[i])), collapse = "_")
# }
...
```{r}
# only thing to add to derpy_crossed is the number of individuals for comparison in the target
# paste(sort(c(derpy_crossed$target.bin[1], derpy_crossed$bin[1])), sep = "_")
# paste(sort(c(derpy_crossed$target.bin[1], derpy_crossed$bin[1])), collapse = "_")
```
...
for (i in 1:length(monomorph_exclude)) { 
  geno.list[[i]] <- data.frame("bin" = names(monomorph_exclude)[i], monomorph_exclude[[i]]) %>% 
    mutate(Var1 = as.character(Var1)) %>% 
    rename(dosage.gt = Var1) %>%
    rename(count = Freq) %>%
    mutate(bin_allele = paste(bin, dosage.gt, sep = "_"))
}
# length(geno.list)
# colnames(geno.list[[1]])
# lapply(geno.list, function(x) length(x))
```

```{r}
# can I just build the data frame straight away?
# how to make all nonredundant pairwise combinations of rows in two data frames?
# if there are only two alleles in a bin, this isn't hard. Just need to reverse the row order of 1 data frame
# since this needs to handle multiallelic cases, that's not going to work.

# use geno.list[[1]] and geno.list[[2]] to find an efficient way to do this
