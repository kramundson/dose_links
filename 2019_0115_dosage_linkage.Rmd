...
dosage_genos <- read_tsv("2019_0107_LOP_250k_dosage_genotypes_40percband.tsv", col_names = T) %>% 
  ...
  filter(chrom == "chr08") # putting the training wheels back on
  # filter(bin %in% c("chr01_0_250000", "chr01_250000_500000")) # training wheels
head(dosage_genos)
```

```{r}
# get number of clusters for each bin from dosage_genos
# test <- dosage_genos %>% 
#   count(bin, dosage.gt)
# test2 <- split(test, f=test$bin)
# test2[[1]]
```
...
> populates 24,000 lookups in <time>. Very slow. How can I speed this up?
...
for (i in 1:nrow(blah)) {
  blah$src.bin.num[i] <- which(unique(dosage_genos$bin) == blah$bin[i])
  blah$target.bin.num[i] <- which(unique(dosage_genos$bin) == blah$target.bin[i])
}
```
